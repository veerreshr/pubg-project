...
$user_check_query = "SELECT * FROM duo WHERE username='".$_SESSION['username']."' and  date='$date'";
...
{        if(isset($_POST['free'])){
    ...
    $user_check_query = "INSERT INTO duo(username,paid,date) values('".$_SESSION['username']."',TRUE,'$date')";
 mysqli_query($db, $user_check_query) ; 
 $_SESSION['duoclaimed']+=1;
      $query="update registration set duoclaimed=". $_SESSION['duoclaimed']." where username='".$_SESSION['username']."'";
    header("location: index.php");
}
     if($_SESSION['correct']){

        $db = mysqli_connect('localhost', 'id12487386_redmealey', 'WctVNUAp@$EuCzqw^Lrg', 'id12487386_pubg') or die("connection failed at begin");
    $user_check_query = "INSERT INTO duo(username,player2,paid,date) values('".$_SESSION['username']."','".$_POST['player2']."',FALSE,'$date')";
 mysqli_query($db, $user_check_query) ; 
...
<body>
    ...
    <div class="container">
        ...
        <?php
         if(($_SESSION['duocount']<=50)and $_SESSION['duoavailable']>0 ){
            echo"<br> <input type='submit' name='free' value='claim free entry'>";
        } else{
            echo "<input type='submit' name='submit' value='correct'";
            if($_SESSION['duocount']>=50){echo "disabled";$_SESSION['correct']=FALSE;}else {$_SESSION['correct']=TRUE;} echo">";
        }
            if($_SESSION['duocount']>=50){
