...
$user_check_query = "SELECT * FROM solo WHERE username='".$_SESSION['username']."' and  date='$date'";
...
if(isset($_POST['submit']))
{
    if(isset($_POST['free'])){
        ...
        $user_check_query = "INSERT INTO solo(username,paid,date) values('".$_SESSION['username']."',TRUE,'$date')";
     mysqli_query($db, $user_check_query) ; 
     $_SESSION['soloclaimed']+=1;
          $query="update registration set soloclaimed=". $_SESSION['soloclaimed']." where username='".$_SESSION['username']."'";
        header("location: index.php");
    }
     if($_SESSION['correct']){
        $db = mysqli_connect('localhost', 'id12487386_redmealey', 'WctVNUAp@$EuCzqw^Lrg', 'id12487386_pubg') or die("connection failed at begin");
    $user_check_query = "INSERT INTO solo(username,paid,date) values('".$_SESSION['username']."',FALSE,'$date')";
 mysqli_query($db, $user_check_query) ; 
...
<body>
    ...
    <div class="container">
        <form action="solopay.php" method="POST">

            <span style="color: blue;">
                <h2>Amount to be paid : &#8377;20</h2>
            </span>
            <br>
          ...
          <?php  if(($_SESSION['solocount']<=100)and $_SESSION['soloavailable']>0 ){
                echo"<br> <input type='submit' name='free' value='claim free entry'>";
            }
            else{
           echo "<input type='submit' name='submit' value='correct'";
            if($_SESSION['solocount']>=100){echo "disabled";$_SESSION['correct']=FALSE;}else {$_SESSION['correct']=TRUE;} echo">";}
